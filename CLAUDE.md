...
│   ├── fetch_announcements.py    # BSE filing feed — the ONLY source here
│   │                             # that postdates the annual report. Reaches
│   │                             # the LLM passes as context and nothing
│   │                             # else; no metric or gate reads it
...
│   │                             # what stops a lender from scoring as a
│   │                             # failing manufacturer on turnover and FCF.
│   │                             # Read by the SCORER as well as the report,
│   │                             # so an entry here withdraws a metric from a
│   │                             # composite and sits in `registry_hash`
│   └── metrics/
...
## Key Patterns
- **Metric registry**: YAML defines metrics in `elements/*.yaml`, Python functions in `builtin/*.py`. Engine auto-discovers both and rejects duplicate metric ids at startup. Adding a metric = 1 YAML entry + 1 function — and the YAML entry must carry a `presentation:` block (unit, direction, bands, low_label, meaning) or **the engine refuses to start**. That block is excluded from both regime hashes by name (`ComputeEngine.HASH_EXEMPT_KEYS`), so declaring how a number reads cannot fragment score history; `validator.py` holds the closed unit and direction vocabularies, and a metric that genuinely has no band declares `bands: []` with a `bands_absent_reason` rather than an invented one.
- **Two outputs per company**: the additive SQGLP composite answers "is this a quality compounder?"; the conjunctive eligibility gates in `registry.yaml` answer "could this plausibly 100x?". A company can score well and still fail a gate — that is the point, not a bug. Gates declare `veto_sources`: if the metric that would emit a veto flag (e.g. `reverse_dcf_growth`) is unavailable, the gate reads `indeterminate` rather than passing on ratios alone.
  - **`fallback_conditions` fire only when every primary condition reads indeterminate**, never to soften one that read and failed. `roiic` is undefined whenever the capital base shrinks, which for a lender in run-off is a permanent state rather than a data gap — so the reinvestment gate returned `indeterminate` for EDELWEISS in every run it would ever have, and an indeterminate verdict caps the displayed action forever. A company was being refused for good on a question the pipeline could not ask it. The fallback (`roe_5yr_avg >= 15`) makes that determinate. **The narrowness is the safety**: for every company whose ROIIC computes, the branch is dead code — ASTRAL never reaches it.
- **A metric that measures nothing for a kind of company does not score it** (`sector_applicability.yaml` → `SQGLPScorer`). The table spent its first life as display data: only the report read it, so a reader was told a lender's DCF margin was meaningless while the composite that lender was *ranked* on had already banked it. The distortion ran both ways at once — EDELWEISS took a perfect 1.0 on `dcf_margin_of_safety` and a perfect 0.0 on `dupont_turnover`, one flattering and one damning, both computed off series that invert for the same reason.
  - **An exclusion leaves BOTH sides of the coverage ratio**, unlike an errored or history-waived metric. Those are evidence that was wanted and not got, and the score is genuinely thinner; an inapplicable metric was never evidence here. Counting it as absent would drive every lender under `low_coverage_threshold` and cap its action for the crime of being a lender — the same penalty in a new costume. EDELWEISS reads 0.923 coverage and carries no `low_data_coverage` flag.
  - **The value and its row survive; the 0-10 judgement is withdrawn, and so are its flags unless the entry says `keep_flags: true`.** Suppression is the default because a metric that measures nothing here says nothing here: JIOFIN's Strengths list opened with "Cash Cow — Strong Cash Conversion" on a company whose operating cash flow was -₹15,439 Cr, off a `cash_conversion` the same run had already withdrawn. The two exceptions argue for themselves in the table — for `debt_equity` and `interest_coverage` the *reading* is sound and only the *threshold* was calibrated for a manufacturer, and suppressing them would remove the only leverage warning a lender's owner gets. **The signal list is built from raw metric results, not from scored details**, so it cannot inherit the scorer's decision and reads `scores["flags_suppressed"]` instead; `build_flags_context` applies the identical rule, so model and reader are never shown different evidence.
  - **Inapplicability is tested before the error branch.** A metric this company was never going to be judged on does not become missing evidence by also failing to compute — and for a lender the two arrive together as a rule: `reverse_dcf_growth` errors with "Negative average FCF" on every lender growing its book, which is *why* the table calls it meaningless. Checked the other way round, JIOFIN's Price coverage read 0.553 instead of 0.627.
  - **The table may be keyed on `sector_industry` as well as `sector`, and both are looked up** (`applicability_labels`). JIOFIN's sector reads "Finance" while its industry reads "Investment Company", so a rule about holding vehicles was unreachable for every company it described. The scorer and the reading layer must call it with the identical labels or the report will describe a metric as withheld that the composite counted.
  - **Miscalibrated is not the same as meaningless**, and the file's `considered and deliberately NOT declared` section is where that line is argued. RoCE reads structurally low for a lender but still orders lenders correctly, so it stays scored; the answer to a bar that does not fit is a second bar that does (`roe_consistency` beside `roce_consistency`), not silence.
  - **An "Investment Company" entry withdraws only the treasury artefacts, not the returns.** JIOFIN carries ₹133,089 Cr of investments against ₹163,467 Cr of assets — 81%, mostly a 6.1% stake in Reliance, which is 65% of its appraised value — so its "78.5% operating margin" is a head office against a dividend cheque and was scoring 100%. That and `dupont_margin` are excluded. `roe_5yr_avg`, `roce_5yr_avg` and `roiic` deliberately are **not**: 0.91% RoE on ₹134,000 Cr of equity is true, material and the central bear point, so withdrawing it would have removed the finding rather than the distortion. What the reader gets instead is `investment_heavy_balance_sheet` and a caveat stating the share, which is what separates "allocates capital badly" from "is a holding vehicle whose return never touches the income statement".
  - **Four metrics exist to keep the lender elements from hollowing out**: `roa_5yr_avg` (the one return measure that reads the same for a lender and a manufacturer), `roe_consistency`, `price_to_book` (every other Price metric divides by a flow, and two of those flows are broken for a lender), and `book_value_cagr_5yr` — per share, so dilution counts. That last one is the signal the old model had no way to see at all: EDELWEISS compounded PAT at +27% while book value per share fell 9.9% a year, and Pass 2 called it "the tell that matters most for a lender".
  - **The table is in `registry_hash` and not in `forward_signal_hash`.** It became scoring regime the moment the scorer read it; a table edit that moved a composite while the hash stayed put would be recorded under the hash of the regime it replaced, and score history is append-only. Zero-weight metrics can never move a composite, so excluding one must not reset any ticker's momentum baseline.
- **Corporate announcements are the only source that postdates the accounts** (`fetch_announcements.py`, `announcements:` in config). Everything else is periodic: the newest annual report can be over a year old, which is how a company whose subsidiary had a **SEBI-cleared IPO in flight** was analysed as though the listing were still hypothetical. Filings reach the LLM passes as context and nothing else — no metric reads them, no gate consults them, no score moves on them, because a subject line is text a company wrote about itself.
  - **The classifier is the real work, and its bias is stated.** A year of filings is ~100 rows and most are compliance chaff; `_NOISE` may only match what is *definitionally* routine, because a false negative costs a line of context and a false positive costs the one filing that changes the answer. Scheduling notices ("Board Meeting Intimation for…") are noise; the outcome filed days later is not.
  - **The RAW feed is cached and the classifier runs on every read**, matching the Screener page cache. Caching the filtered frame would make the patterns immutable for the life of a cache entry, and which filings survived would differ between two tickers purely by when each was last fetched.
- **A holding company's consolidated ratios describe no business it owns** (`sector.group_structure`). Screener's `sector_industry` breadcrumb names one outright and nothing read it. EDELWEISS consolidates a lending book in deliberate run-off (₹18,000 Cr → ₹600 Cr) with fee businesses compounding 27-63%; blended, that is 2.6% revenue growth, and the model filed it as a Growth Trap — a verdict describing an average no segment resembles. The quadrant is still computed and still shown; what travels with it is `consolidated_group_ratios` and a caveat both passes read. The flag maps to `FORWARD_SIGNALS_ELEMENT` because `quality_growth_quadrant` carries weight 0.0 (KTD6) — not because a group structure is a forward signal, but because that is the bucket for "said something, scored nothing".
...
- **Forward signals** (Phase 2, `v05 §7.1–7.3`, `§11`): four additive capabilities, none of which may move a score.
  ...
  - **Momentum refuses to diff across regimes** (`trajectory.py`, KTD5). Rows partition by `config_hash`, and separately by `synthetic`; a backfilled row never supplies the headline. Every figure states the actual day gap it spans. **Insufficient history is a distinct outcome, never a zero** — a zero delta means flat, no delta means unknown, and they look identical in a table. Kept out of `score_history.py`, whose contract is append and read.
    - **The headline comes from the regime the NEWEST row was scored under**, which is a different question from "whichever organic step ends latest" and gives a different answer on exactly the run that matters: the first one after a regime change writes a single row under the new hash, which cannot form a step, so the freshest step in the log still belongs to the *retired* regime. That is how a report came to print a composite of 3.54 above a trajectory reading "4.20 → 4.16" — two numbers off two different rulers, with nothing on the page saying so. Refusing to bridge regimes *inside* a step was only half the rule; this is the same error one level up, committed by the choice of which step to show. Retired regimes keep their steps in `regimes`, and the insufficient-history reason **names the regime change** rather than reporting thin history, because "nobody has scored this twice" and "the ruler changed" call for opposite responses.
...
- **A number can be arithmetically correct and still not be a reading** (`UNSCORABLE_FLAGS` in `metrics/base.py`). A CAGR from ₹45 Cr to ₹3,521 Cr is 328%/yr and is a fact about JIOFIN's demerger date; six such metrics scored a flat 1.0 carrying 47% of its Growth element, and the trailing PEG off the same base was 38% of all scored Price weight. The report then printed *"0.29x is below 1.0 — the golden rule for 100-baggers... the valuation appears justified and attractive"* about a share on 78x earnings.
  - **The value is kept and shown; what the flag withdraws is its vote.** The scorer waives it (`waived: "not_a_reading"`, weight renormalises, coverage records the gap because the evidence genuinely is missing) and `EligibilityEvaluator` refuses to gate on it. **Both, or neither** — a figure too unreliable to score is too unreliable to admit a company through a 100x gate, and the two layers reading one list is what stops them drifting.
  - **Flags derived from an artefact are artefacts too**, and that is a second suppression rule from the sector one: "Attractive Trailing PEG" survived the first pass and led JIOFIN's Strengths off the very 0.29x the report had just refused. The flag naming the artefact is the one thing kept.
  - The test is on the **base** (below 5% of the ending value), not on the resulting rate. Both catch the case; only this one can say why, and R7 needs a reason a reader can act on rather than "the number looked too big".
- **A bonus issue is not dilution and must not read as book-value destruction** (`_restated_share_counts`). It moves reserves into equity capital: net worth unchanged, share count up, every holder compensated in shares — so book value per share falls by arithmetic while nothing has happened to the owner. `book_value_cagr_5yr` charged for it on **13 of the 22 cached tickers**, four catastrophically (JIOFIN -92.8%/yr, IGIL -87.9%, TARSONS -54.0%, IXIGO -46.4%). The discriminator is the share-count jump *relative to the net-worth jump*: a re-denomination leaps while net worth barely moves, a genuine issuance brings cash in step and is left alone, because real dilution SHOULD lower book value per share. It cannot separate a raise priced below ~40% of book, which is stated in the docstring rather than left to be discovered; a net-worth threshold was tried instead and was worse, since BSE's 2025 bonus landed in a year net worth rose 51%. A window containing negative net worth is refused outright — there is no compounding through a deficit.
...
- **Data fetches bypass the machine's system proxy by default** (`data_fetcher/proxy.py`, `fetching.use_system_proxy`). Every source here is a public financial site, and a system proxy on a personal machine is more often an ad blocker or VPN than required egress — Magic Lasso Adblock runs as a macOS packet-tunnel extension that advertises itself on an **ephemeral loopback port**, so `HTTP_PROXY` is set for every process with nobody configuring it and the port changes on each restart, which is why `NO_PROXY` is not a durable answer and this is a setting. While such a proxy reloads its filter lists it refuses the CONNECT handshake, and every HTTPS fetch in that window dies three times over with `Tunnel connection failed: 400 Bad Request` — three `urllib3` retries against a failure retrying cannot fix.
  - **Three network paths, three mechanisms.** `BaseFetcher`'s session sets `trust_env = False` (an empty `session.proxies` does NOT override the environment — requests merges env proxies in at request time unless trust_env is off). `bse_codes.py` is the one call outside that session. yfinance and jugaad-data take no session of ours and read `os.environ` themselves, so they run inside a `bypassed()` context manager that removes both cases of the variables and restores them in a `finally`.
  - **The LLM transport is deliberately untouched.** `llm_layer/transport.py` allowlists the proxy variables into the headless CLI's environment on purpose, for the corporate-egress case; one module's fetching preference has no business reaching that decision. Set `use_system_proxy: true` on a machine that reaches the internet only through its proxy — and note that turning it on also restores `REQUESTS_CA_BUNDLE` and `.netrc`, which travel with it rather than pretending to be independent.
...
- **Money parked is not money spent** (`operating_free_cash_flow`, `_helpers.py`). `CFO + CFI` is free cash flow for a company whose investing line is plant. For a cash-rich one CFI also carries mutual funds, bonds and deposits — cash a buyer of the whole company would get back. CAPLIPOINT grew financial investments ₹848 Cr over five years against ₹495 Cr of plant and CWIP, and the consequences ran through four metrics and one gate: average FCF read ₹10.8 Cr for a company holding ₹2,875 Cr of liquid assets, the DCF returned ₹43 against a ₹2,561 price (-98.3%, scored 0), the reverse DCF **pinned at its +50% ceiling**, and the `reverse_dcf_overpriced` veto that fired off it FAILED the entry-price gate whose own PEG conditions had passed. Adjusted: FCF yield -0.30% → +1.08%, FCF-positive years 4/10 → 9/10, reverse DCF 50.0 (saturated) → 33.9 (a real measurement, and still a veto — the company genuinely is priced for more growth than it has delivered).
  - **The correction is an estimate and says so.** The balance sheet gives a year-end *stock*, so a rise is inferred to be cash going in — but a rise can also be a mark-to-market gain, which consumed no cash. Metrics that used it carry `fcf_adjusted_for_treasury`. Only **increases** are added back: a fall is cash coming out, and netting it off would hide a company liquidating its treasury to cover an operating shortfall.
  - **ROIIC divides by OPERATING capital** (`_capital_employed(exclude_treasury=True)`), and the consistency argument is what settles it: NOPAT is derived from `operating_profit`, which excludes `other_income`, so the denominator must exclude the assets generating that income. CAPLIPOINT read 14.86% against a 15.0 gate — **failed by 0.14pp for holding cash** — and reads 23.19% on capital actually deployed. `capital_partly_undeployed` fires when a quarter or more of retained capital never reached the business, because "earns well on what it deployed while parking the rest" is a different proposition from "earns well on everything".
  - Lenders and investment companies are unaffected: for them `investments` IS the business, and every metric here is already withdrawn by `sector_applicability.yaml`.
- **A transition stub is not a year** (`STUB_PERIOD_LABEL`, `_get_annual_rows`). A company changing its financial year end files one shortened period, and Screener carries its length in the label — `Mar 20169m` is nine months of Caplin Point, not the year 20169. It starts with the new month, so every filter that checks only the leading month keeps it: a ten-year window counted it as a year and the report's own snapshot printed "169m" as a column heading. `stub_period_labels` is exposed so a surface can say a period was dropped.
...
- **Prompt labels and units come from the registry, never from a list in `checklist.py`** (`_registry_display`, `_label_for`). Every metric already declares its `name` and `presentation.unit` in YAML the engine validates at startup; a second copy for the prompt is a copy that drifts, and it drifted three times, each putting a confident wrong sentence in front of the model. `promoter_holding_trend` was labelled "Promoter Holding Δ (3yr) pp" while the metric returns the LEVEL. `cash_conversion` was labelled "Cash Conversion (CFO/PAT)" while it computes **OCF/EBITDA** — so Caplin Point's thesis led with "CFO/PAT of 61.5%, two-fifths of profit has not converted", set a kill-trigger at "CFO/PAT below 70%", and printed a table on the same page showing CFO/PAT at 80% for two straight years. `reinvestment_rate` was labelled `%` while it returns a multiple, so the model reported a contradiction that did not exist. A test walks the id lists against the registry, so an id the registry drops fails loudly rather than rendering as a title-cased guess.
- **Sector tailwind is classified on all three breadcrumbs, most specific first** (`study_labels`). Screener gives `sector_industry`, `sector` and `sector_broad`, and the study's list matches whichever it happens to match — asking with only the middle one lost companies the study plainly covers. Caplin Point's `sector` reads "Pharmaceuticals & Biotechnology", which whole-phrase matching finds in neither "Healthcare" (strong) nor "Pharma" (moderate), while its `sector_broad` is literally "Healthcare" and was never consulted; it scored `unknown`. Narrower wins: a pharma company is pharma before it is healthcare, so it takes `moderate_tailwind` rather than the broad group's `strong_tailwind`. Screener's own spellings are aliased into the list rather than the matcher being loosened.
- **A product approval is not a stake change** (`fetch_announcements._MATERIAL`). Pharma companies announce every clearance as "receipt of final approval by our subsidiary", and bare `subsidiary` sat in the stake pattern — so all eight Caplin Steriles USFDA approvals reached the model tagged "[Stake / acquisition]". `product_approval` is now its own category, ordered ahead of `stake_change`, and `subsidiary` counts toward a stake change only when paired with a word meaning ownership moved. The model caught the mislabelling and warned the reader, which is the ensemble working and not a reason to leave it.
...
- **The provider is a transport, not a contract** (`llm_layer/transport.py`). Every LLM call — Pass 1, Pass 2, the Stage 1.5 extraction — leaves through `build_transport(config)`, resolving `llm.provider` to either `anthropic` (the SDK against API credits, the default) or `claude_cli` (headless `claude -p`, billed to the Max subscription's headless pool). Both receive the **identical rendered prompt** and both return text through the same `_parse_json_response`, so nothing downstream — parsing, validation, grounding, the sidecar, the action guard — can tell which ran except by reading usage metadata. Everything else follows from that rule: the CLI's `--json-schema` is deliberately unused (it would be a second statement of each pass's output schema beside the prompt's, and they would drift invisibly), the sidecar's version block gains **no** `provider` field (its identity is source text + schema + prompt + model — a grounded reading stays a grounded reading, so a provider switch must not re-spend the corpus), and `llm.provider` sits in **neither regime hash**, all three pinned by tests. Model selection stays orchestrator-side, so `--deep` means "swap to Opus" identically on both. Every failure is one `TransportError`, caught where `anthropic.APIError` used to be, returning the same `{"error": ..., "pass": ...}` dict.
  - **The child environment is the load-bearing control, and it is an allowlist.** `.env` sets `ANTHROPIC_API_KEY` and dotenv loads it into `os.environ`; the CLI gives an env key precedence over subscription OAuth, and a whole family of other variables (`CLAUDE_CODE_USE_BEDROCK`/`_VERTEX`/`_FOUNDRY`/`_GATEWAY`, `AWS_BEARER_TOKEN_BEDROCK`, `ANTHROPIC_BEDROCK_BASE_URL`, `ANTHROPIC_CUSTOM_HEADERS`, …) reroutes a call to a 3P provider billing its own credentials. This was a three-name denylist and every one of those survived it. **A denylist is the wrong shape for a control whose failure is silent and financial**: it must be re-audited against every CLI release and `config.yaml` pins a *minimum* version, not a maximum. So `_child_env` builds the child up from `INHERITED_ENV_KEYS` — PATH/HOME/USER/SHELL/TMPDIR/TERM/LANG/TZ, `LC_*` by prefix, the proxy and CA variables a machine behind corporate egress needs, `NODE_OPTIONS`, and `CLAUDE_CONFIG_DIR` so a relocated `~/.claude` does not read as logged out — and nothing else. `BILLING_ROUTING_PREFIXES` is a redundant second gate kept for the day someone widens that list. The asymmetry is the argument: starving the child fails loudly with the CLI's own message and one name to add, while a leaked routing variable bills the wrong pool in silence. The test asserts the **property**, including that a routing variable nobody has heard of yet cannot get through.
  ...
  - **Cost on this path is not comparable to the API path's, and the usage block says so.** The envelope's `input_tokens` **excludes cache reads** — a call moving ~37K tokens reported `2` — so the cache counts travel beside it or the CLI path shows a phantom efficiency. **They only defend anything as far as they are rendered**: carried per-call and then aggregated nowhere, a two-pass run that moved ~35K tokens printed `1604 tokens` on every surface and read as forty times more token-efficient than the API path at twice the price — the same misreading, defeated one layer above where it was fixed. `_summarize_usage` totals them into `total_cache_read_input_tokens`/`total_cache_creation_input_tokens` and the derived `total_cached_input_tokens`, **emitted only when some entry carried the field** (a `0` would read as "wrote its whole prefix fresh", the expensive case, not the absent one), and console, report and sweep footer all print `(+N cached)` beside the count.
  - **A failed call is not a free call.** The harness prefix is billed before the model reads a word of ours, so a call that fails after that has already spent. `TransportError` carries `cost_usd` — read off an `is_error` envelope, off a no-result envelope, and off stdout that a non-zero exit still printed — and `_call_api` appends a usage entry *before* returning the error dict. Otherwise the spend leaves `usage_summary()` entirely: invisible to the sweep's `--ceiling` across the dozens of serial calls a full sweep makes, and reported as $0.0000 for the ticker that paid it. The entry's token counts are `None` with `tokens_basis: "unknown"`, never 0 — the call may have moved tens of thousands of tokens before failing — and `failed_calls` in the summary is present only when there is something to correct, its presence being the statement that the totals beside it are short. A failure the transport could not price adds nothing to the cost and does not vote on `cost_basis`: "estimated" would claim a $0 estimate nobody made.
  - **`CHARS_PER_TOKEN` lives in `transport.py` and the sweep imports it.** It was measured in `sweep.py` first (4 is the Sonnet 4.6 figure; the Claude 5 family tokenizes ~30% higher, so 4 under-prices by about a third — wrong in the direction a spend ceiling cares about) and the CLI transport's own fallback estimate was then written at 4 anyway. That fallback fires exactly when the envelope's usage block is unreadable, i.e. when the estimate is the only number anyone has, and it feeds `estimate_cost()` and therefore the ceiling. Every fresh call pays a ~$0.033 harness-prefix floor that amortizes only inside a resumed session (`--resume`, measured at $0.0030 for a follow-up turn; session-per-company is v2 of the transport, not v1), and Claude Code writes every prompt token at 1-hour-TTL cache-write rates — 2× standard input — so the optimized path converges to ≈1.7–1.8× the API path per company. `estimated_cost_usd` **keeps its name** because the sweep's `--ceiling` meters on it, but on this path it holds actuals; `cost_basis: "actual" | "estimated" | "mixed"` states which, the same honesty pattern `friction.basis` uses. A ceiling calibrated against API pricing therefore trips far sooner here, and per-ticker cost stops being a per-ticker fact.
