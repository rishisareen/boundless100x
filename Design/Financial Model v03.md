...
**Version:** 3.0 — Final Consolidated Design
**Framework:** SQGLP (Size, Quality, Growth, Longevity, Price)

**Source Material:**
- **100 Baggers** — Christopher Mayer (2015). US-focused study of stocks returning 100-to-1. Key themes: owner-operators, buy-right-and-hold-on, stock buybacks, competitive moats.
- **Motilal Oswal 19th Wealth Creation Study (2014)** — The original SQGLP framework paper. Defines the 5 elements, the 4-lever growth decomposition, value migration as dominant business theme, and quantitative criteria for 100x hunting.
- **Motilal Oswal 30th Wealth Creation Study (Dec 2025)** — Introduces the QGLP 25-question checklist, NTD/MTD era thesis, compounder identification methodology, Quality-Growth Matrix, sector tailwind classification, and the India MTD Portfolio.
- **100 Baggers — Indian Context (User Notes)** — Synthesized application of all sources to Indian markets with specific quantitative thresholds per SQGLP element.
...
2. [Data Sources & Fetching (Stage 1)](#2-data-sources--fetching-stage-1)
3. [Automated Peer Discovery (Stage 1.5)](#3-automated-peer-discovery-stage-15)
4. [Extensible Compute Engine (Stage 2)](#4-extensible-compute-engine-stage-2)
5. [SQGLP Metrics — Detailed Specification](#5-sqglp-metrics--detailed-specification)
6. [Sector & Macro Context](#6-sector--macro-context)
7. [LLM Analysis Layer (Stage 3)](#7-llm-analysis-layer-stage-3)
8. [Output & Report Generation (Stage 4)](#8-output--report-generation-stage-4)
9. [Service Layer — The GUI-Ready API](#9-service-layer--the-gui-ready-api)
...
16. [Future Work: Interactive GUI](#16-future-work-interactive-gui)
...
│  elements/*.yaml define all metrics              │
│  engine.py auto-discovers and runs each function │
...
### 4.2 Architecture: Split Metric Registry

Instead of one monolithic YAML file (error-prone at 50+ metrics), metrics are organized into per-element files with auto-discovery.

```
compute_engine/
└── metrics/
    ├── registry.yaml              # Master: element weights + global config ONLY
    ├── elements/                  # One file per SQGLP element (auto-discovered)
    │   ├── size.yaml              # S metrics
    │   ├── quality_business.yaml  # Q (business) — RoCE, margins, DuPont, cash conv.
    │   ├── quality_management.yaml# Q (management) — promoter, pledge, owner-operator
    │   ├── growth.yaml            # G — CAGR, 4-lever, quality grade, consistency
    │   ├── longevity.yaml         # L — CAP proxy, streaks, stability, reinvestment
    │   ├── price.yaml             # P — PE, PEG, trailing PEG, DCF, reverse DCF
    │   └── composite.yaml         # Cross-element: Quality-Growth Matrix
    ├── custom/                    # User drop-in metrics (auto-discovered)
    │   └── my_forensics.yaml      # e.g., Benford's Law, Piotroski F-Score
    ├── presets/                   # Screening presets
    │   ├── compounders.yaml       # Dec 2025 Motilal Oswal methodology
    │   └── hidden_gems_100x.yaml  # 2014 SQGLP 100x hunting criteria
    ├── base.py                    # MetricResult dataclass
    ├── validator.py               # YAML schema validation on startup
    └── builtin/                   # Implementation modules
        ├── __init__.py
        ├── profitability.py       # RoCE, RoE, OPM, NPM, Cash Conversion, DuPont
        ├── growth.py              # CAGR, 4-lever decomposition, quality grade
        ├── valuation.py           # P/E, PEG, trailing PEG, EV/EBITDA, DCF
        ├── leverage.py            # D/E, Interest Coverage
        ├── efficiency.py          # Working Capital Days, CCC, Asset Turnover
        ├── size.py                # Market cap, institutional, turnover, owner-operator
        ├── longevity.py           # Consistency, streaks, stability, CAP proxy
        └── composite.py           # Quality-Growth Matrix classification
```

### 4.3 Master Registry (`registry.yaml`) — Now Minimal

The master file contains only element weights and global scoring config. All metric definitions live in `elements/*.yaml`.
...
scoring_defaults:
  ...
  missing_data_score: null       # Exclude from scoring if data unavailable
  min_data_years: 3              # Minimum years of data required for a metric
```

### 4.4 Element Files — Per-SQGLP Metric Definitions

Each element file is focused (~60-100 lines), easy to edit, and independently validatable.
...
metrics:
  ...
  institutional_holding:
    ...
    scoring:
      mode: "range_optimal"
      optimal_range: [1, 10]     # Sweet spot: discovered but not crowded
      weight: 0.25
...
metrics:
  ...
  # DuPont Decomposition (QGLP Checklist Q6)
  dupont_margin:
...
#### `elements/quality_management.yaml`
...
metrics:
  promoter_holding_trend:
    ...
    display: { format: "{:.1f}% (Δ{:+.1f}pp)", section: "management_assessment", peer_compare: true }
  ...
  promoter_pledge:
    ...
    display: { format: "{:.1f}%", section: "management_assessment", peer_compare: true, red_flag_threshold: 10 }
...
#### `elements/growth.yaml`

```yaml
# Reference: "4 dimensions of earnings growth — Volume, Price, Operating & Financial Lever"
# (Motilal Oswal 2014 Study)
element: growth
...
metrics:
  ...
  dcf_margin_of_safety:
    ...
    params: { projection_years: 10, terminal_growth: 0.04, discount_rate: 0.12 }
  ...
  reverse_dcf_growth:
    ...
    display: { format: "{:.1f}% implied vs {:.1f}% actual", section: "valuation_analysis" }
...
#### `elements/composite.yaml`

```yaml
# Cross-element derived metrics (from Dec 2025 Quality-Growth Matrix)
element: composite

metrics:
  quality_growth_quadrant:
    name: "Quality-Growth Matrix Position"
    module: "builtin.composite"
    function: "compute_qg_quadrant"
    inputs: ["financials"]
    params:
      quality_metric: "roce_5yr_avg"
      quality_threshold: 15
      growth_metric: "pat_cagr_5yr"
      growth_threshold: 15
    scoring:
      mode: "categorical"
      categories:
        "true_wealth_creator": 10
        "quality_trap": 4
        "growth_trap": 4
        "wealth_destroyer": 1
      weight: 0.0    # Display-only, not scored into SQGLP composite
    display: { format: "{}", section: "executive_summary", peer_compare: true }
```

### 4.5 Auto-Discovery Engine
...
from metrics.validator import validate_registry

class ComputeEngine:
    def __init__(self, registry_dir: str = "compute_engine/metrics"):
        ...
        # Validate on startup — fail fast on YAML errors
    ...
    def _run_metric(self, metric_id: str, config: dict, data: dict) -> MetricResult:
        ...
        if not required.issubset(available):
            return MetricResult(value=None, error=f"Missing: {required - available}")
...
### 4.6 Registry Validator

```python
# compute_engine/metrics/validator.py
...
### 4.9 Adding a New Metric — Complete Workflow
...
Or, for custom metrics, create a new file `custom/my_efficiency.yaml`:
...
### 6.1 Sector Tailwind Classification
...
The **QGLP 25-Question Checklist** (from Dec 2025 Wealth Creation Study) structures the LLM's qualitative analysis. Of the 25 questions, ~15 are partially or fully answered by computed metrics. The LLM focuses on the remaining ~10 that require true qualitative judgment from annual report text.

### 7.2 QGLP Checklist Integration
...
└── prompts/
    ├── pass1_checklist.yaml     # 25-question structured template
    ├── pass1_qualitative.txt    # Prompt using checklist answers
    ├── pass2_synthesis.txt      # Receives Q1-Q25 answers as context
...
## 8. Output & Report Generation (Stage 4)
...
## 9. Service Layer — The GUI-Ready API
...
python cli.py screen --preset compounders       # Motilal Oswal Dec 2025 methodology
python cli.py screen --preset hidden_gems_100x  # SQGLP 2014 100x hunting
...
**Why 3 LLM passes instead of 1 big call?**
Each pass has a focused scope with a specific output schema, producing more reliable and structured output. A single large prompt with all data tends to produce generic analysis. Splitting also lets you use cheaper models for simpler tasks (Pass 3 with Haiku) and skip passes when data isn't available (Pass 1 without annual reports).

**Why a metric registry instead of hardcoded modules?**
Adding a metric should be a 5-minute task (YAML entry + function), not a multi-file refactor. The registry pattern also makes scoring, peer comparison, and report generation self-configuring — they read the registry and auto-adapt.

**Why split the registry into per-element YAML files?**
A monolithic registry with 40+ metrics becomes error-prone (one YAML indentation error breaks everything), creates merge conflicts, and is cognitively overwhelming. Per-element files are focused (~80 lines each), independently editable, and validated on startup. Custom metrics drop into `custom/` with zero changes to existing files. The auto-discovery engine scans `elements/` and `custom/` directories at startup.

**Why the QGLP 25-question checklist in the LLM layer?**
The Motilal Oswal Dec 2025 report provides a professionally-tested assessment framework. Instead of open-ended LLM prompts that produce variable output, the structured checklist ensures systematic coverage of all quality dimensions. The key insight: ~15 of 25 questions are pre-answered by computed metrics, so the LLM only does genuine qualitative judgment on the remaining ~10 questions. This is both higher quality and more cost efficient.
...
It enforces clean separation. Without it, business logic leaks into CLI argument parsing, making it impossible to reuse. With it, adding a Streamlit frontend is literally `result = service.analyze_company("ASTRAL")` — zero refactoring of the pipeline.

---

## 16. Future Work: Interactive GUI
...
### 16.3 GUI-Readiness Already Built In
...
### 16.4 Technology Options When Ready
...
### 16.5 GUI Feature Roadmap (When Implemented)
...
### 16.6 Data Layer Evolution for GUI
